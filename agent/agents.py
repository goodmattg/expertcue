...
from utils.motion import get_foot_vel
import torch
...
class Agent2x(BaseAgent):
    ...
    def forward(self, data):
        ...
        if self.use_triplet:
            ...
            losses["m_tpl1"] = self.triplet_weight * self.tripletloss(
                motionvecs[2], motionvecs[0], motionvecs[1]
            )
            losses["m_tpl2"] = self.triplet_weight * self.tripletloss(
                motionvecs[3], motionvecs[1], motionvecs[0]
            )
            losses["b_tpl1"] = self.triplet_weight * self.tripletloss(
                staticvecs[2], staticvecs[0], staticvecs[1]
            )
            losses["b_tpl2"] = self.triplet_weight * self.tripletloss(
                staticvecs[3], staticvecs[1], staticvecs[0]
            )
        else:
        ...
        if self.use_footvel_loss:
            losses["foot_vel"] = 0
            for i, target in enumerate(targets):
                losses["foot_vel"] += self.footvel_loss_weight * self.mse(
                    get_foot_vel(outputs[i], self.foot_idx),
...
class Agent3x(BaseAgent):
    ...
    def forward(self, data):
        ...
        if self.use_triplet:
            outputs, motionvecs, bodyvecs, viewvecs = self.net.cross_with_triplet(
                inputs
            )
            losses["m_tpl1"] = self.triplet_weight * self.tripletloss(
                motionvecs[2], motionvecs[0], motionvecs[1]
            ...
            losses["b_tpl1"] = self.triplet_weight * self.tripletloss(
                bodyvecs[2], bodyvecs[0], bodyvecs[1]
            )
            losses["b_tpl2"] = self.triplet_weight * self.tripletloss(
                bodyvecs[3], bodyvecs[1], bodyvecs[0]
            )
            losses["v_tpl1"] = self.triplet_weight * self.tripletloss(
                viewvecs[2], viewvecs[0], viewvecs[1]
            )
            losses["v_tpl2"] = self.triplet_weight * self.tripletloss(
                viewvecs[3], viewvecs[1], viewvecs[0]
            )
        else:
            outputs = self.net.cross(inputs[0], inputs[1])
