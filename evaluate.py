...
from utils.motion import preprocess_motion2d, postprocess_motion2d, openpose2motion
from utils.core import ensure_dir, pad_to_height
...
def handle3x(config, args):
    ...
    # get input
    input1 = openpose2motion(
        args.vid1_json_dir, scale=scale1, max_frame=args.max_length
    )
    input2 = openpose2motion(
        args.vid2_json_dir, scale=scale2, max_frame=args.max_length
    )
    ...
    if args.out_dir is not None:
        ...
        if args.render_video:
            ...
            motion2video(
                ...
                h3,
                w3,
                os.path.join(save_dir, "input3.mp4"),
                color3,
                args.transparency,
                fps=args.fps,
                save_frame=args.save_frame,
...
def main():
    ...
    parser.add_argument(
        "-o", "--out_dir", type=str, default="./outputs", help="output saving directory"
    )
    parser.add_argument(
        "--render_video", type=bool, default=True, help="whether to save rendered video"
    )
    parser.add_argument("--fps", type=float, default=25, help="fps of output video")
    parser.add_argument(
        "--save_frame", action="store_true", help="to save rendered video frames"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--color3", type=str, default="#ff8b06#ffb431#ffcd9d", help="color3"
    )
    parser.add_argument(
        "--disable_smooth",
        action="store_true",
