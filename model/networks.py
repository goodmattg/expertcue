...
class AutoEncoder2x(nn.Module):
    def __init__(
        ...
        assert (
            mot_en_channels[0] == de_channels[-1]
            and mot_en_channels[-1] + body_en_channels[-1] == de_channels[0]
        )
...
class AutoEncoder3x(nn.Module):
    def __init__(
        self, mot_en_channels, body_en_channels, view_en_channels, de_channels
    ):
        super(AutoEncoder3x, self).__init__()

        assert (
            mot_en_channels[0] == de_channels[-1]
            and mot_en_channels[-1] + body_en_channels[-1] + view_en_channels[-1]
            == de_channels[0]
        )
        ...
        self.body_encoder = Encoder(
            body_en_channels,
            kernel_size=7,
            global_pool=F.max_pool1d,
            convpool=nn.MaxPool1d,
            compress=True,
        )
        self.view_encoder = Encoder(
            view_en_channels,
            kernel_size=7,
            global_pool=F.avg_pool1d,
            convpool=nn.AvgPool1d,
            compress=True,
        )
        self.decoder = Decoder(de_channels)
    ...
    def cross(self, x1, x2):
        ...
        out1 = self.decoder(
            torch.cat(
                [m1, b1.repeat(1, 1, m1.shape[-1]), v1.repeat(1, 1, m1.shape[-1])],
                dim=1,
            )
        )
        out2 = self.decoder(
            torch.cat(
                [m2, b2.repeat(1, 1, m2.shape[-1]), v2.repeat(1, 1, m2.shape[-1])],
                dim=1,
            )
        )
        out121 = self.decoder(
            torch.cat(
                [m1, b2.repeat(1, 1, m1.shape[-1]), v1.repeat(1, 1, m1.shape[-1])],
                dim=1,
            )
        )
        out112 = self.decoder(
            torch.cat(
                [m1, b1.repeat(1, 1, m1.shape[-1]), v2.repeat(1, 1, m1.shape[-1])],
                dim=1,
            )
        )
        out122 = self.decoder(
            torch.cat(
                [m1, b2.repeat(1, 1, m1.shape[-1]), v2.repeat(1, 1, m1.shape[-1])],
                dim=1,
            )
        )
        out212 = self.decoder(
            torch.cat(
                [m2, b1.repeat(1, 1, m2.shape[-1]), v2.repeat(1, 1, m2.shape[-1])],
                dim=1,
            )
        )
        out221 = self.decoder(
            torch.cat(
                [m2, b2.repeat(1, 1, m2.shape[-1]), v1.repeat(1, 1, m2.shape[-1])],
                dim=1,
            )
        )
        out211 = self.decoder(
            torch.cat(
                [m2, b1.repeat(1, 1, m2.shape[-1]), v1.repeat(1, 1, m2.shape[-1])],
                dim=1,
            )
        )
