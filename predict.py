...
def handle2x(config, args):
    ...
    # get input
    if args.prebundle:
        input1 = prebundle_openpose_to_motion(
            args.vid1_json_dir, scale=scale1, max_frame=args.max_length
        )
        input2 = prebundle_openpose_to_motion(
...
def main():
    ...
    parser.add_argument("--prebundle", action="store_true", help="load OpenPose keypoints from [15, 2, T] numpy (.npy) matrix")
