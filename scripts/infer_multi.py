...
def parse_args():
    ...
    # )
    parser.add_argument(
        "--im-dir", dest="im_folder", help="input image folder", default=None, type=str
    )
    parser.add_argument(
        "--rpn-pkl", dest="rpn_pkl", help="rpn model file (pkl)", default=None, type=str
    )
    parser.add_argument(
        "--rpn-cfg",
        dest="rpn_cfg",
...
def check_args(args):
    assert (args.rpn_pkl is not None and args.rpn_cfg is not None) or (
