...
from utils.core import cycle
from agent import get_training_agent
from utils.visualization import visulize_motion_in_training
import torch
import os
...
def main():
    ...
    parser.add_argument(
        "-g", "--gpu_ids", type=int, default=0, required=False, help="specify gpu ids"
    )
    parser.add_argument(
        "--disable_triplet",
        action="store_true",
        default=False,
        help="disable triplet loss",
    )
    parser.add_argument(
        "--use_footvel_loss",
        action="store_true",
        default=False,
        help="use use footvel loss",
    )
    parser.add_argument(
        "--vis", action="store_true", default=False, help="visualize output in training"
    )
    ...
    train_loader = get_dataloader(
        "train", config, config.batch_size, config.num_workers
    )
    ...
    for e in range(config.nr_epochs):
        ...
        train_tb.add_scalar(
            "learning_rate", tr_agent.optimizer.param_groups[-1]["lr"], clock.epoch
        )
