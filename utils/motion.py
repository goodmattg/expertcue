...
def trans_motion2d(motion2d):
    ...
    # adding velocity
    velocity = np.c_[np.zeros((2, 1)), centers[:, 1:] - centers[:, :-1]].reshape(
        1, 2, -1
    )
...
def get_local3d(motion3d, angles=None):
    ...
    local_y = np.cross(
        horizontal, local_z
    )  # bugs!!!, horizontal and local_Z may not be perpendicular
...
def openpose2motion(json_dir, scale=1.0, smooth=True, max_frame=None):
    ...
    for path in json_files:
        with open(path) as f:
            ...
            joint = np.array(jointDict["people"][0]["pose_keypoints_2d"]).reshape(
                (-1, 3)
            )[:15, :2]
            if len(motion) > 0:
...
def get_foot_vel(batch_motion, foot_idx):
    return (
        batch_motion[:, foot_idx, 1:]
        - batch_motion[:, foot_idx, :-1]
        + batch_motion[:, -2:, 1:].repeat(1, 2, 1)
    )
